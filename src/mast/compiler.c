...
static void compile_instruction(VM, WriterState *ws, MASTNode *node) {
    if (ISTYPE(vm, node, ws->types->Op)) {
        ...
        unsigned short call_op  = c->op == 0 ? MVM_OP_invoke_v : MVM_OP_ncinvoke_v;
        ...
        if (ISTYPE(vm, c->result, ws->types->Local)) {
            ...
            switch (ws->cur_frame->local_types[l->index]) {
                case MVM_reg_int64:
                    call_op = c->op == 0 ? MVM_OP_invoke_i : MVM_OP_ncinvoke_i;
                    res_type = MVM_operand_int64;
                    break;
                case MVM_reg_num64:
                    call_op = c->op == 0 ? MVM_OP_invoke_n : MVM_OP_ncinvoke_n;
                    res_type = MVM_operand_num64;
                    break;
                case MVM_reg_str:
                    call_op = c->op == 0 ? MVM_OP_invoke_s : MVM_OP_ncinvoke_s;
                    res_type = MVM_operand_str;
                    break;
                case MVM_reg_obj:
                    call_op = c->op == 0 ? MVM_OP_invoke_o : MVM_OP_ncinvoke_o;
        ...
        ws->bytecode_pos += 2;
        if (call_op != MVM_OP_invoke_v && call_op != MVM_OP_ncinvoke_v)
